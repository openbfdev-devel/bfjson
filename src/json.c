...
next_state(const char code, enum json_state state)
{
    ...
    for (count = 0; count < BFDEV_ARRAY_SIZE(transition_table); ++count) {
                  ...
                  struct bfjson_node **root)
{
    ...
    struct bfjson_node *nstack[PASER_NODE_DEPTH], *node = NULL;
    ...
    for (walk = buff; *walk; ++walk) {
        ...
        if (nnpos < cnpos && nnpos < 0)
            ...
            struct bfjson_node *parent = node;
        ...
        if (is_struct(nstate)) {
            switch (*walk) {
                case '[':
                    bfjson_set_array(node);
                    break;

                case '{':
                    bfjson_set_object(node);
                    break;
...
static int
encode_depth(const struct bfjson_node *parent, char *buff,
             int size, int len, unsigned int depth)
{
    #define json_sprintf(fmt, ...) len += snprintf(buff + len, bfdev_max(0, size - len), fmt, ##__VA_ARGS__)
    struct bfjson_node *child;
    unsigned int count;

    if (bfjson_test_array(parent))
        json_sprintf("[\n");
    else if (bfjson_test_object(parent))
        json_sprintf("{\n");
    else
        return 0;
    ...
    bfdev_list_for_each_entry(child, &parent->child, sibling) {
        for (count = 0; count < depth + 1; ++count)
            json_sprintf("\t");
        if (bfjson_test_object(parent))
            json_sprintf("%s: ", child->name);
        if (bfjson_test_array(child) || bfjson_test_object(child))
            len = encode_depth(child, buff, size, len, depth + 1);
        else if (bfjson_test_number(child))
            json_sprintf("%ld,\n", child->number);
        else if (bfjson_test_string(child))
            json_sprintf("\"%s\",\n", child->string);
        else if (bfjson_test_null(child))
            json_sprintf("null,\n");
        else if (bfjson_test_true(child))
            json_sprintf("true,\n");
        else /* bfjson_test_false(child) */
            json_sprintf("false,\n");
    }
...
export void
bfjson_release(const struct bfdev_alloc *alloc, struct bfjson_node *root)
{
    struct bfjson_node *node, *tmp;
    ...
    bfdev_list_for_each_entry_safe(node, tmp, &root->child, sibling) {
        ...
        if (bfjson_test_string(node))
            bfdev_free(alloc, node->string);
        else if (bfjson_test_array(node) || bfjson_test_object(node)) {
            bfjson_release(alloc, node);
            continue;
        }
