...
static void json_dumpinfo(struct bfjson_node *parent, unsigned int depth)
{
...
int main(void)
{
    struct bfjson_node *jnode;
    int length, retval;
    char *buff;

    retval = bfjson_decode(NULL, json_test, &jnode);
    if (retval)
        return retval;
    ...
    printf("json encode:\n");
    length = bfjson_encode(jnode, NULL, 0);

    buff = malloc(length);
    if (!buff)
        return 1;

    length = bfjson_encode(jnode, buff, length);
    fwrite(buff, length, 1, stdout);
    free(buff);

    bfjson_release(NULL, jnode);
    return retval;
}
